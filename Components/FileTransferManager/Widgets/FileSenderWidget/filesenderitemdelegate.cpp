...
FileSenderItemDelegate::FileSenderItemDelegate(QObject *parent) : QItemDelegate(parent)
{

}

void FileSenderItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
//    const QString fileupState = index.data(FileItemInfo::FileSenderUploadRole).toString();
    ...
    if(option.state.testFlag(QStyle::State_Selected)) {
        ...
        name_str = filepath;
        // 获取filepath的矩形
        nameOrpathRect = painter->fontMetrics().boundingRect(filepath);
...
QSize FileSenderItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(index)
